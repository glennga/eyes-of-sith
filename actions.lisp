...
(defun introduction ()
  ...
  (format t (concat-strings (list
	  ...
	  "You are a Sith apprentice who has been tasked with killing another Sith in~%"
	  ...
	  "be promoted to Lord. Be warned, facing Nox in your current state is... unwise~%"
  ...
  (progn (cond ((eq *selected-npc* '^darth@nox)
		(if (and (have '^mysterious@amulet)
			 (or (have '^blue@lightsaber)
			     (have '^red@lightsaber)))
		    (progn (format t (concat-strings (list ; win game if have amulet and lightsaber
						      "--------------------------------------------------------------------------~%"
		    ...
		    (progn (format t (concat-strings (list ; otherwise, lose game try to face him too soon 
						      "--------------------------------------------------------------------------~%"
		...
		(progn (format t (concat-strings (list ; kill the trooper action
						  "--------------------------------------------------------------------------~%"
						  "Your eyes glow red as your slowly raise your arm upward. \"I'm ~%"
						  "sorry lord-\" are the last words of this soldier as his body ~%"
						  "goes limp and drops to the ground. You take the Kolto Pack ~%"
						  "from his body.~%"
						  "--------------------------------------------------------------------------~%")))
		       (pickup '^kolto@pack)
		       (kill-npc *selected-npc*)))
		...
		(progn (format t (concat-strings (list ; kill the trooper action
						  "--------------------------------------------------------------------------~%"
						  "You extend both of your arms forward as blinding arcs of energy ~%"
						  "shoot out of your fingertips. The soldier now lies on the ~%"
						  "ground, with small arcs of lightning radiating from his body.~%"
						  "--------------------------------------------------------------------------~%")))
		       (pickup '^blaster)
		       (kill-npc *selected-npc*)))
		...
		(progn (format t (concat-strings (list ; kill the young acolyte action
						  "--------------------------------------------------------------------------~%"
						  "With no hesitation, you draw your saber and strike down the~%"
						  "innocent young Acolyte. You take the Training Blade from his body.~%"
						  "--------------------------------------------------------------------------~%")))
		...
		(format t (concat-strings (list ; harm the jedi action
					   "--------------------------------------------------------------------------~%"
					   "\'It looks like we\'re winning now.\' The Jedi drops to the ~%"
					   "floor, still alive and gasping for air.~%"
					   "--------------------------------------------------------------------------~%"))))
		...
		(progn (format t (concat-strings (list ; free the jedi action
						  "--------------------------------------------------------------------------~%"
						  "You unlock the cage and tell her to run. She hands you her~%"
						  "lightsaber. She replies, \'Thank you. May the force be with~%"
						  "you.\' and runs off.~%"
						  "--------------------------------------------------------------------------~%")))
		...
		(progn (format t (concat-strings (list ; kill the jedi action
						  "--------------------------------------------------------------------------~%"
						  "You swell with anger hearing that phrase. You draw your saber~%"
						  "and push your saber through the his knees. \'How about now?!\'~%"
						  "you scream. Within minutes, the Jedi's screams slowly fade away.~%"
						  "--------------------------------------------------------------------------~%")))
		       (kill-npc *selected-npc*)))
		...
		(progn (format t (concat-strings (list ; peaceful ghost action
						  "--------------------------------------------------------------------------~%"
						  "You raise your hands in the air and try to reason with the ghost.~%"
						  "The entity then enters your mind consumes your spirit.~%"
						  "Your eyes roll to the inside of your head and your limp body~%"
						  "drops to the stone.~%~% Try again!~%"
						  "--------------------------------------------------------------------------~%")))
		       (reload))))
...
(defun B ()
  ...
  (progn (cond ((eq *selected-npc* '^darth@nox)
		...
		(progn (format t (concat-strings (list ; friendly trooper action
						  "--------------------------------------------------------------------------~%"
						  "You request the Kolto Pack from the trooper. He replies, \'Of~%"
						  "course my lord.\' You now have the Kolto Pack.~%"
						  "--------------------------------------------------------------------------~%")))
		       (pickup '^kolto@pack)))
	       ((eq *selected-npc* '^imperial@trooper@2)
		(progn (format t (concat-strings (list ; friendly trooper action
						  "--------------------------------------------------------------------------~%"
						  "You request the blaster from the trooper. He replies, \'Of ~%"
						  "course my lord.\' You now have the Blaster.~%"
						  "--------------------------------------------------------------------------~%")))
		       (pickup '^blaster)))
	       ((eq *selected-npc* '^young@acolyte)
		(format t (concat-strings (list ; friendly young acolyte action
					   "--------------------------------------------------------------------------~%"
					   "He turns on his practice droid and shows himself deflecting the~%"
					   "blaster fire. \'Wow that was pretty good! Keep it up!\'"
					   "--------------------------------------------------------------------------~%"))))
	       ((eq *selected-npc* '^jedi@3)
		(progn (format t (concat-strings (list ; kill jedi action
						  "--------------------------------------------------------------------------~%"
						  "You draw your saber from your hip and turn on your saber. For~%"
						  "a brief moment, the only sound you hear is the humming of your~%"
						  "blade. You extend your blade through the cage to pierce her~%"
						  "chest. You pick up her Blue Lightsaber."
						  "--------------------------------------------------------------------------~%")))
		       (pickup '^blue@lightsaber)
		       (kill-npc *selected-npc*)))
	       ((eq *selected-npc* '^apprentice@harken)
		(progn (format t (concat-strings (list ; kill harken action
						  "--------------------------------------------------------------------------~%"
						  "You extend both arms and shock Harken with all of your lightning.~%"
						  "This catches him by surprise and he falls to the ground, trying to~%"
						  "catch his breath. He stands back up, weakened and angry. He~%"
						  "charges at you and you quickly draw your saber through his chest.~%"
						  "He falls to the ground, dead. You take the Red Lightsaber from his~%"
						  "body.~%"
						  "--------------------------------------------------------------------------~%")))
		...
		(if (have '^kolto@pack)
		    (progn (format t (concat-strings (list ; no kill w/ kolto pack action
						      "--------------------------------------------------------------------------~%"
						      "You draw your blade and swing it in the direction of the ghost...~%"
						      "but it goes right through him. He chokes you and infects you with~%"
						      "some sort of poison. Luckily you have your Kolto Pack! You inject~%"
						      "it and escape before the ghost notices.~%"
						      "--------------------------------------------------------------------------~%")))
		    ...
		    (progn (format t (concat-strings (list ; killed w/o kolto pack action
						      ...
						      "You draw your blade and swing it in the direction of the ghost...~%"
						      "but it goes right through him. He chokes you and infects you with~%"
						      "some sort of poison. You can crawl around the temple, but help is~%"
						      "too far away. Your world fades to black.~%"
						      "--------------------------------------------------------------------------~%")))
			   (reload)))))
	 '@)) ; ensure that nothing extra is printed
